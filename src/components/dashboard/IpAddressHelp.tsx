  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          <div
            className="w-full max-w-lg rounded-2xl border border-border bg-card p-5 shadow-2xl animate-scale-in"
            onClick={e => e.stopPropagation()}
            role="dialog"
            aria-label="How to find your camera IP address"
          >
            <div className="flex items-start justify-between gap-3 mb-4">
              <h3 className="text-[20px] font-bold">How to get the IP address</h3>
              <button
                onClick={() => setOpen(false)}
                aria-label="Close"
            ...
            <div className="flex gap-4 items-center">
              <div className="relative w-[120px] h-[210px] flex-shrink-0 rounded-[20px] border-4 border-border bg-background overflow-hidden">
                <div className="absolute top-1 left-1/2 -translate-x-1/2 w-10 h-1.5 rounded-full bg-border" />
                <div
                  className="absolute inset-x-0 top-6 transition-transform duration-500 ease-out"
                  style={{ transform: `translateY(-${step * 46}px)` }}
                >
                  {STEPS.map((s, i) => (
                    <div
                      key={s.title}
                      className={`h-[46px] mx-2 mb-1 rounded-lg border px-2 flex items-center gap-1.5 transition-colors ${
                        i === step ? 'border-primary bg-primary/15' : 'border-border bg-secondary/30'
                      }`}
                    >
                      <s.icon className={`w-4 h-4 ${i === step ? 'text-primary' : 'text-muted-foreground'}`} />
                      <span className="text-[11px] font-semibold leading-tight">{s.title}</span>
                    </div>
                  ))}
                </div>
                <div className="absolute bottom-2 inset-x-2 rounded-md bg-primary/20 border border-primary/40 px-1.5 py-1">
                  <span className="text-[10px] font-bold text-primary">192.168.18.98</span>
                </div>
              </div>
